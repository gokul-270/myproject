...
#define DEBUGWITHOUTLIMITSWITCH  true
		...
		zero_offset_[joint_id] = EncoderOffSet[joint_id] = CanBusController.GetPositionEstimate(LocalCanID);
			...
			ROS_INFO("%s: Read  Zero offset at homing_position for jointId : %d from encoder: %lf",joint_id, ReadJointOffSet) ;
                        }
                } 
		...
		++No_Of_Joint_initialised_to_home_;
                ...
                if (No_Of_Joint_initialised_to_home_ == NoOfJointsToBeInitialised)
		...
		{ // the calling flag is used for blocking the execution of read and write during the initialisation service
			...
			if ((No_Of_Joint_initialised_to_home_ == NoOfJointsToBeInitialised) && (Block_Read_Write_During_Initialization == 0))
			{
				...
				if (ConditionToReadWriteJoint)
				{
					...
					*/
					long ReadMotorRotationFromMotorZero =  joint_current_position  ; 
					long ReadMotorRotationFromLimitSwitch = ReadMotorRotationFromMotorZero + EncoderOffSet[joint_id] ; //EncoderOffset  should be in Rotations 
					long ReadJointPositionFromLimitSwitch = (ReadMotorRotationFromLimitSwitch / transmission_factor_[joint_id] ) / encoder_resolution_[joint_id] * direction_[joint_id]; 
					long ReadJointPositionFromJointZero = ReadJointPositionFromLimitSwitch - JointOffSet[joint_id];
					...
					if( ODriveREAD_DEBUG ) { 
					    ...
					    long oldJointPosition = (joint_current_position / transmission_factor_[joint_id]) / encoder_resolution_[joint_id];
	...
	{   static int counthz = 0 ;
		...
		if ( ++counthz == 100 ) {
			...
			Testposition  = Testposition * -1 ;
                //SimpleMotorTest(CanBusController,0, Testposition );
                //SimpleMotorTest(CanBusController,1, Testposition );
		}
		...
		if ((No_Of_Joint_initialised_to_home_ == NoOfJointsToBeInitialised) && (Block_Read_Write_During_Initialization == 0))
		{
			for (std::size_t joint_id = 0; joint_id < num_joints_; ++joint_id)
			{
				...
				// odrive_serial_interface_[joint_id]->ReadProperty(ODRV_CURRENT_POSITION_R, &joint_current_pos);
				//  DONOT READ L2 which is  joint_id =3 && height_scan_enable == False   Feb13 2023 by Mani Radhakrishnan
				bool ConditionToReadWriteJoint = (joint_id != 2) || (height_scan_enable == true) ;
				if (ConditionToReadWriteJoint)
				{
					...
					{
						if (DEBUGWRITE) ROS_INFO("Writing to odrive for joint =: %s", joint_names_[joint_id].c_str());
						if (DEBUGWRITE) ROS_INFO("ODriveHWInteface::write() %s: Command: %lf, Last Command: %lf", joint_names_[joint_id].c_str(), (float)joint_position_command_[joint_id], (float)last_joint_position_command_[joint_id]);
						...
						if (true) {
							...
							if (DEBUGWRITE) ROS_INFO(" %s:Zero Offset at Homing_Pos to be added to Joint Value : %lf", joint_names_[joint_id].c_str(), zero_offset_[joint_id]);
							if (DEBUGWRITE) ROS_INFO("joint_position_command_ := %d", (int)joint_position_command_[joint_id]);
                            /*
                            ...
                            */
							long MotorRotationFromJointZero = joint_position_command_[joint_id] * transmission_factor_[joint_id] * encoder_resolution_[joint_id] * direction_[joint_id]; 
							long MotorRotationFromLimitSwitch = MotorRotationFromJointZero + (JointOffSet[joint_id]  * transmission_factor_[joint_id] * encoder_resolution_[joint_id] * direction_[joint_id]); 
						        long MotorRotationFromMotorZero = MotorRotationFromLimitSwitch - EncoderOffSet[joint_id] ; // EncoderOffSet will be negative value 
							output_value = MotorRotationFromMotorZero ;	
							...
							if (DEBUGWRITE) {
							    long old_output_value = (joint_position_command_[joint_id] * transmission_factor_[joint_id] * encoder_resolution_[joint_id] * direction_[joint_id]) + zero_offset_[joint_id];
							...
							if (DEBUGWRITE) ROS_INFO("Command Sent to CanNodeID  %d, MotorId : %d\n",CanID[joint_id], joint_id);
