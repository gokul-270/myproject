...
//rt_task::rt_task(pthread_t * thread, std::function<void* (void*)>* func, void* arg, long period, int priority)
//rt_task::rt_task(pthread_t * thread, std::function<void* (void*)> (*func), void* arg, long period, int priority) 
...
//rt_task::rt_task(pthread_t * thread, void *(*func) (void *), void* arg, long period, int priority)
{
  ...
  if( DEBUG ) printf(" %s >> period of rt_task in ns : %ld \n", __func__, period) ;
  if( DEBUG ) printf(" %s >> priority of rt_task : %ld \n",__func__, priority) ;
  if( DEBUG ) sleep(3) ;
}
...
void rt_task::create_rt_task()
{
  ...
  printINFO(" Completed ::  ret = pthread_attr_init(&attr);\n");

  /* Set a specific stack size  */
  ret = pthread_attr_setstacksize(&attr, PTHREAD_STACK_MIN);
  if (ret != 0)
  handle_error_en(ret, "pthread_attr_setstacksize");
  printINFO("  Completed ::  ret = pthread_attr_setstacksize(&attr, PTHREAD_STACK_MIN);\n");

  /* Set scheduler policy and priority of pthread */
  ret = pthread_attr_setschedpolicy(&attr, SCHED_FIFO);
  if (ret != 0)
  handle_error_en(ret, "pthread_attr_setschedpolicy");
  printINFO("  Completed ::    ret = pthread_attr_setschedpolicy(&attr, SCHED_FIFO)\n");
  ...
  printINFO("  Completed ::   ret = pthread_attr_setschedparam(&attr, &param\n");
  
  /* Use scheduling parameters of attr */
  ret = pthread_attr_setinheritsched(&attr, PTHREAD_EXPLICIT_SCHED);
  if (ret != 0)
  handle_error_en(ret, "pthread_attr_setinheritsched");
  printINFO("  Completed ::  ret = pthread_attr_setinheritsched(&attr, PTHREAD_EXPLICIT_SCHED)\n"); 

  ret = pthread_create(this->_thread, &attr, this->helper,  this);
  if (ret != 0)
  handle_error_en(ret, "pthread_create");
  printINFO("  Completed ::   ret = pthread_create(this->_thread, &attr, this->helper,  this);\n");
...
void *rt_task::periodic_task()
{
 ...
 //if( DEBUG )  sleep( 10 ) ;
 if( DEBUG ) printf(" %s period of tasks in ns is : %ld \n", __func__, this->_period_ns) ;
