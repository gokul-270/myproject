...
namespace odrive_control
{
    class ODriveHWInterface : public ros_control_boilerplate::GenericHWInterface
    {
        public:
            ...
            static  void readexp(void *args);
            /* Write the commands from topics to robot hardware */
            virtual void write(ros::Duration &elapsed_time);
            static void writeexp(void *args);
        ...
        private:
            ...
            std::vector<int> CanID; // Added to include a CANID for every joint . 15JUN2023
            std::vector<int> axis_id_;
            std::vector<int> limit_switch_id_;
            std::vector<float> transmission_factor_;
            //JointOffset is the offset of the Zero position of the joint w.r.t the limit switch (used as an absolute reference) in meters or radians
            // This is a design parameter which is read from the YAML file for each joint 
            std::vector<double> JointOffSet;
            std::vector<double> zero_offset_home_;
            ...
            // EncoderOffset is the value (in motor rotations unit) of the limit switch position. 
            // THis value is read during the joint initialisation and can be stored in the .yaml file for furture use
            std::vector<double> EncoderOffSet;
