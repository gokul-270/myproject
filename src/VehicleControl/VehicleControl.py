# History
# Last Date When Code Updated : 26MAY2023
...
# 26May2023 i) Made to run with automatic control mode in both direction by having only one direction movement from row to row
#           ii) the error in the manual pivotal mode is fixed by getting them to closed loop before moving
import os
import can
#import cantools
import numpy
...
LOGFILEPATH = "/home/ubuntu/pragati/outputs/marcLogFile.txt"

def WriteLogFiles(txtmsg):
 # TODO Writeto a LogFile instead of print
  if (LOGFILEENABLED) :
     print("writing to logfile :", LOGFILEPATH)
 ...
 # that is the user is not actively controllling the vehicle  
  if  (   (abs(XValue - JoyStickMidValue     ) < JoyStickResolution ) and
      ...
      if (CurrentTime - LastOperationTime) > IdlingTime :
         ...
         VehicleIsSetToIDLE = True
         printINFOJOYSTICK( "Idling time Hit") 
...
def Execute():
   ...
   while True:
      ...
      VehicleControlExecuteStartTime = time.time()
      #PreviousSystemModeValue = SystemModeValue
      SystemModeValue = FilteredVehicleControlMode()
      #print( "PreviousSystemModeValue  :", PreviousSystemModeValue)
      ...
      if (SystemModeValue == VEHICLE_IN_AUTOMATIC_MODE) :
         ...
         if ( ModeChange or DirectionChange ):
            printINFOJOYSTICK( " MODE CHANGE IN AUTOMATIC MODE") 
            SettingTrapTrajValues()
            VehicleCanBusInterface.SetDriveMotorsToPositionControlMode()
            #VehicleDirection = FilteredGetDirection()
            ...
            if (VehicleDirection == MANUALMODE_RIGHTDIRECTION) :
               VehicleCanBusInterface.MoveSteeringMotorsTo90degreerotationRight()
               printINFOJOYSTICK( " Automatic Mode MOVING  Vehicle in RIGHT Direction") 
            else :
               VehicleCanBusInterface.MoveSteeringMotorsTo0degreerotation()
               printINFOJOYSTICK( " Automatic Mode MOVING  Vehicle in NEUTRAL Direction") 
            
            printINFOJOYSTICK( " Automatic Mode MOVING  Vehicle Direction") 
            #time.sleep(10)
            if( not CheckForOperationToComplete() ) :
               printINFO(" AutomaticControlMode Operation Failed")
               exit()
         printINFOJOYSTICK( " Executing AutomaticCOntrol Mode") 
         AutomaticControlMode()        
         printINFOJOYSTICK("VehicelControl.Execute:: Vehicle is automatic Mode ")
         printINFOJOYSTICK( "Time in AutomaticModeControlMode  :  {}".format(1000*(time.time() - VehicleControlExecuteStartTime ))) 
      else:
         #if ( PreviousSystemModeValue != SystemModeValue ):
         if ( ModeChange or DirectionChange ):
            SettingTrapTrajValues()
            VehicleCanBusInterface.SetSteeringMotorsToClosedLoopControl()
            printINFOJOYSTICK( " MODE CHANGE IN MANUAL MODE") 
            if (VehicleDirection == MANUALMODE_LEFTDIRECTION):
            ...
            printINFOJOYSTICK( "Manual Mode PIVOTING  Vehicle Direction") 
            #time.sleep(10)
            if( not CheckForOperationToComplete() ) :
               printINFO(" Manual Mode operation Failed")
               exit()
        # USE THE JOYSTICK FOR DRIVE AND STEERING Motors
         printINFOJOYSTICK( " Executing VehicleControlWithJoyStick") 
         VehicleState = VehicleControlWithJoyStick()
