# History
# Last Date When Code Updated : 27JUN2023
...
# 27JUN2023 i)added the AbsoluteEncoderfunction to get the steering wheelstraight at initialization.
...
IMUEnable = False
if (IMUEnable) :
  import IMUReadExperiment #Commenting out for running in fiel
...
def FilteredAnalogEncoderValue():
   ListEncoderValue=[]
   for i in range (0,20):
     EncoderValue= ReadAnalogEncoder.ReadEncoder()
     ListEncoderValue.append(EncoderValue) 
#     time.sleep(0.1)
   #print(ListEncoderValue)
   EncoderValue_List = Counter(ListEncoderValue)
   Filtered_EncoderValue= (EncoderValue_List.most_common()[0][0])*100 
   #print(Filtered_EncoderValue)

   return Filtered_EncoderValue

def ConvertEncoderValueToMotorAngle():
    ...
    AnglePerEncoderValueInRotation=360/Maximumum_EncoderValue
    Current_EncoderValue = FilteredAnalogEncoderValue()
    if (abs( current_EncoderValue - CenterPointValue)>10):
       if(Current_EncoderValue > CenterPointValue):
          Current_correctionAngle =  AnglePerEncoderValueInRotation*(current_EncoderValue-CenterPointValue)
       if(Current_EncoderValue < CenterPointValue):
          Current_correctionAngle =  AnglePerEncoderValueInRotation*(CenterPointValue-current_EncoderValue)

    return(Current_correctionAngle)

def StaighteningAtInitial():
   RightWheelRotation,LeftWheelRotation,BackWheelRotation=ConvertEncoderValueToMotorAngle()
   VehicleCanBusInterface.SetSteeringMotorsToClosedLoopControl()
   VehicleCanBusInterface.AutoMaticStraighteningSteeringMotorsTo(RightWheelRotation,LeftWheelRotation,BackWheelRotation)
...
def AutomaticControlMode():
   ...
   printINFO("VehicleCantrol.AutomaticControlMode :: Automatic Movement")
   VehicleDirection = FilteredGetDirection()
   ...
   if (ButtonStatus == ButtonPressed):
      ...
      if (VehicleDirection ==  NONBRAKE_MANUAL_MODE) :
         ...
         printINFO( "VehicleControl.AutomaticControlMode :: moving Forward")
      elif (VehicleDirection == MANUALMODE_LEFTDIRECTION) :
         VehicleCanBusInterface.MoveAutomaticMovement(BackwardMovement)
         printINFO( "VehicleControl.AutomaticControlMode :: moving Backward")
