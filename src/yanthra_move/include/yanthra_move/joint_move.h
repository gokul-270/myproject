...
//#include <dynamixel_msgs/JointState.h> //TODO:MR
...
class joint_move
{
	...
	{
		joint_state = *msg;
		//if(joint_state.load == 0xff) //TODO:dynamixel may have the parameter called load bu dynamixel_msgs might not have
...
joint_move::joint_move(ros::NodeHandle nh, std::string name, int ODriveJointID)
{
	joint_pub = nh.advertise<std_msgs::Float64>(name + "command", 2);
 	//joint_status = nh.subscribe("joint_states", 10, &joint_move::joint_state_cb, this);//TODO:MR
