...
def WaitOnSignal():
    ...
    print("CottonDetect WaitOnSignal : Received a Signal")
    if (sig == signal.SIGUSR1):
            #PrintCottonDetailsTxtFile()
            print("CottonDetect : Got SIGUSR1", file = logfile )
            #DetectCotton()
            DetectionOutputRequired = True
            #continue
    if (sig == signal.SIGHUP):
            #self.load_configuration()
            print("CottonDetect : Got SIGHUP", file = logfile )
            #continue
    
    if (sig == signal.SIGTERM):
            #logging.info('Got SIGTERM, exiting
            print("CottonDetect : Got SIGTERM", file = logfile )
